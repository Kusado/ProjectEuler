  ...
  public static class Helpers {
    ...
    public static string LongSum(string a, string b, int step = 8) {
      ...
      foreach (int i in result) {
        sb.Append(IntToString(i, step));
      }
      string sResult = sb.ToString().TrimStart('0');
      return sResult;
    }

    public static string LongMult(string a, string b, int step = 4) {
      var aInts = GetIntsFromString(a, step);
      var bInts = GetIntsFromString(b, step);
      var result = MultIntArr(aInts, bInts, step);
      StringBuilder sb = new StringBuilder();
      foreach (int i in result) {
        sb.Append(IntToString(i, step));
      }
    ...
    public static int[] MultIntArr(int[] a, int[] b, int step) {
      if (step > 4) throw new ArgumentException("Больно дофига разрядов для инта, уменьшите размерность входных данных (результат умножения может не поместиться в int)");
      int[] result = new int[a.Length + b.Length];
      int razr = (int)Math.Pow(10, step);//Число разрядов при текущем шаге.

      //Перемножаем входные данные
      for (int indB = b.Length - 1; indB >= 0; indB--) {
        for (int indA = a.Length - 1; indA >= 0; indA--) {
          int z = b[indB] * a[indA];
          int indR = indA + indB + 1;
          result[indR] += z;
        }
      }

      //Складываем в столбик результат предыдущих вычислений.
      for (int i = result.Length - 1; i >= 0; i--) {
        if (result[i] > 0 && result[i] / razr > 0) {
          int val = result[i];
          result[i - 1] += val / razr;
          result[i] = val % razr;
        }
    ...
    public static string IntToString(int input, int charCount) {
      var str = input.ToString();
      StringBuilder sb = new StringBuilder();
      for (int i = 0; i < charCount; i++) {

        int indX = i + str.Length - charCount;
        string x = indX >= 0 ? str[indX].ToString() : "0";
        sb.Append(x);
      }
      return sb.ToString();
    }
